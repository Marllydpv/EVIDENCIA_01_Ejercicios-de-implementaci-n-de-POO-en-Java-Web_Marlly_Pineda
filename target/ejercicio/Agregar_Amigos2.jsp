<head>
  <%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Agregar amigos</title>
  </head>
  <link href="resources/assets/css/pag3.css" rel="stylesheet">
  <body>
   
  
  
<nav>
    <div class="logo">
      <img src="resources/img/ic.png" alt="">
    </div>
    <ul class="menu">
        <li><a href="objeto?url=redflex">inicio</a></li>
      <li><a href="objeto?url=agregarh2">Agregar historias</a></li>
      <li><a href="objeto?url=cuenta2">Cuenta</a></li>
     
    </ul>
  </nav>
  <  <div class="card-container">
    <div class="card">
      <img src="resources/img/img1.png" alt="">
      <div class="card-info">
        <h2>Agregar Amigos</h2>
        <p>Comparte con nuevos amigos</p>
        <a href="">Compartir Historia</a>
      </div>
    </div>
    <div class="card">
        <img src="resources/img/i.png" alt="">
        <div class="card-info">
          <h2>Eliminar Amigos</h2>
          <p>+ +</p>
          <a href="#">Eliminar historia</a>
        </div>
      </div>
  </div>
  
  <footer>
    <p>Conoce nuevas personas</p>
    <p> Marlly Pineda </p>
  </footer>
  </body>